        ...
        if start == end:
            ...
            with warnings.catch_warnings():
                ...
                tensor = torch.frombuffer(
                    mv[start:end], dtype=_TYPES[info["dtype"]]
                ).view(info["shape"])
                ...
                setattr(
                    ...
                    comfy.memory_management.TensorFileSlice(
                        f, threading.get_ident(), data_base_offset + start, end - start
                    ),
                )
        ...
        try:
            if comfy.memory_management.aimdo_enabled:
                ...
                with safetensors.safe_open(
                    ckpt, framework="pt", device=device.type
                ) as f:
                    sd = {}
                    for k in f.keys():
...
def tiled_scale_multidim(
    ...
    for b in range(samples.shape[0]):
        ...
        out_div = torch.zeros(
            [s.shape[0], 1] + mult_list_upscale(s.shape[2:]), device=output_device
        )
